...
from eaia.chat_model import ChatModel
...
async def update_general(state: ReflectionState, config, store: BaseStore):
    """Update the general reflection prompt based on the feedback and trajectory"""

    chat_model = ChatModel("openai", "o1", disable_streaming=True)

    reflection_model = chat_model.get_model()
...
class MultiMemoryInput(MessagesState):
    """State for updating multiple memory types"""
...
async def determine_what_to_update(state: MultiMemoryInput):
    """Determine what to update based on the feedback and trajectory"""

    chat_model = ChatModel("openai", "gpt-4o", disable_streaming=True)
